...
2. **扫描 Issue 状态**：优先通过 `gh-issues` 获取本人负责的 open Issue，按优先级(p0>p1>p2)排序。
3. **执行写操作**：每次运行至少完成 1 个 `gh-issues` 写操作——Issue 评论澄清需求 / 标签维护 / Issue 描述补充验收标准。
4. **标注状态流转**：对每个处理过的 Issue 更新状态标签；需求不明确时设 status:blocked 并写明阻塞原因、需要谁确认、最晚确认时间。
5. **输出交接信息**：明确下游角色（技术总监/高级程序员）的待办事项和交接要点。

## 质量规则

1. **每次必须写操作**：每轮执行至少产出 1 条 `gh-issues` 写操作（Issue 评论或标签/字段更新），纯读取不产出视为失败。
